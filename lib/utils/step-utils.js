...
const { normalizeStepArgsForProviderPolicy } = require("../provider-policy");
...
function stepArgsFromCommand(command, stepId, selectedUrls, format) {
  ...
  return {
    needsInput: false,
    args: normalizeStepArgsForProviderPolicy(args, { format }),
  };
}
...
function buildStepArgs(step, index, selectedUrls, format) {
  ...
  if (normalized.tool === "fetch") {
    ...
    return {
      needsInput: false,
      args: normalizeStepArgsForProviderPolicy(
        ["fetch", url, "--format", format || "json"],
        { format },
      ),
    };
  }
  ...
  if (normalized.tool === "map") {
    ...
    return {
      needsInput: false,
      args: normalizeStepArgsForProviderPolicy(["map", url], { format }),
    };
  }
  ...
  if (normalized.tool === "exa-search") {
    return {
      needsInput: false,
      args: normalizeStepArgsForProviderPolicy(
        ["exa-search", normalized.query || normalized.question],
        { format },
      ),
    };
  }
  ...
  if (SEARCH_LIKE_STEP_TOOLS.has(normalized.tool)) {
    return {
      needsInput: false,
      args: normalizeStepArgsForProviderPolicy(
        [normalized.tool, normalized.query || normalized.question],
        { format },
      ),
    };
  }
